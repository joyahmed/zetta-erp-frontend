...
const AccountsReceivableForm = () => {
		...
		const fetchInvoices = async () => {
			try {
				const response = await fetch(
					`${window.zettaSettingsData.api_url}accounts/receivable`
				);
				if (!response.ok) {
	...
	const submitUrl =
		action === 'create'
			? `${window.zettaSettingsData.api_url}accounts/receivable`
			: `${window.zettaSettingsData.api_url}accounts/receivable/edit/${id}`;

	const page = 'zetta_erp_receivable';
	...
	const {
		...
		`/wp-json/accounts/receivable/${id}`,
		submitUrl,
		page
	);
